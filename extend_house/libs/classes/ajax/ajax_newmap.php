    ...
    {   
		...
		switch($entry){
          ...
          foreach($coclass1 as $k => $v){
            $count = 0;
            if($query = $db->fetch_one('SELECT COUNT(*) cnt FROM ' . $tblprefix . atbl($chid) . ' WHERE ccid1 = ' . $k )){
                $count = $query['cnt'];
            }
          ...
          switch($this->_get['type']){
            case 'chushou':
				...
				$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM {$tblprefix}".atbl($chid)." a INNER JOIN {$tblprefix}archives_" . $chid . " c ON a.aid=c.aid " . $whereStr . $limitStr;
				$query = $db->query($sql);
                ...
                if(!empty($projcode)){
                  if($projData = $db->fetch_one("SELECT  * FROM {$tblprefix}".atbl(4)." a INNER JOIN {$tblprefix}archives_4 c ON a.aid=c.aid WHERE a.aid = \"" . $projcode . "\" AND (c.leixing = 0 OR c.leixing = 2)" )){
                	...
                	$HouseData['project']['projurl'] = $projData['arcurl'];
		  ...
		  case 'CommunityPointData';//小区显示(出售,出租)
            ...
            switch($this->_get['type']){
                ...
                case 'chuzu':
					...
					if($chid == 3){
						$fycnt = $db->fetch_one("select count(*) cnt from " . $tblprefix . atbl(3) ." where pid3 = " . $row['aid']);
					}else{
						$fycnt = $db->fetch_one("select count(*) cnt from " . $tblprefix . atbl(2) ." where pid3 = " . $row['aid']);
					}
		  ...
		  case 'marker'://独立页120在使用该处		 
			...
			while($row = $db->fetch_array($query)){
				cls_ArcMain::Url($row,-1);								
				$salepic = 2;
				$project = array('projcode'=>$row['aid'],'projname'=>$row['subject'],'salepic'=>$salepic,'price'=>@$row['dj'],'px'=>$row['dt_1'],'py'=>$row['dt_0'],'url'=>@$row['arcurl'],'url1'=>@$row['arcurl1'],'url2'=>@$row['arcurl2'],'url3'=>@$row['arcurl3'],'url4'=>@$row['arcurl4'],'url5'=>@$row['arcurl5'],'url6'=>@$row['arcurl6']);
				$communityData['project'][] = $project;
			}                   
			return $communityData;
		  break;
