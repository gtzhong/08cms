...
if(!submitcheck('bsubmit')){
	...
	trbasic('预约刷新数量','',$curuser->info['freeyys'].' 次','');
...
<script type="text/javascript">
	...
	function checkbox(form,yu_e,cash,each_pay){//找出全部被选的checkbox
		...
		j = j - num;
		if( yu_e - j >= 0){
			_mes = '本次预约刷新设置应扣除预约刷新次数：<font color="red">' + j + '</font>条。剩余预约刷新次数：<font color="red">' + (yu_e - j) + '</font>条,剩余金额<font color="red">' + cash + '</font>元。';
		}else if( cash - (j - yu_e) * each_pay >= 0){
			
			_mes = '本次预约刷新设置应扣除预约刷新次数：<font color="red">' + yu_e + '</font>条,同时扣除金额<font color="red">' + parseFloat(((j - yu_e) * each_pay).toFixed(2).toString()) + '</font>元，您剩余的刷新余额为<font color="red">0</font>次，金钱为<font color="red">' + parseFloat((cash - (j - yu_e) * each_pay).toFixed(2).toString()) + '</font>';
		}else{		
			alert('本次预约刷新设置应扣除预约刷新次数：' + yu_e + '条,同时扣除金额' + parseFloat((each_pay*(j - yu_e)).toFixed(2).toString()) + '元。\n您总的刷新余额为' + yu_e + '次，金额为' + cash + '元，不足以支付本次刷新数目，请充值或者重新设置。');
			return false;
		}
		if(_mes){
