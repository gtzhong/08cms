...
if(!submitcheck('bsubmit')){
	aheader();	
	$excel->ShowFieldsTable($chid,$cuid,"请选择导出数据的项目",$where_str,"?entry=extend$extend_str");
	a_guide('exportexcel');
}else{
	if(count($fmdata)<2){
		aheader(); 
	 	cls_message::show('请选择导出项目。',"?entry=$entry$extend_str&chid=$chid&cuid=$cuid&aid=$aid&q=$q&p=$p");
	}
