...
if($action == 'gaoji'){
	...
	if(!submitcheck('bsubmit')){
		...
		trbasic('每天可预约房源条数',"rulenew[totalnum]",empty($rule['totalnum']) ? 0 : $rule['totalnum'],'text',array('validate' => makesubmitstr("rulenew[totalnum]",1,0,1,'','int'),'guide' => '一天可以对n条房源进行预约刷新设置，以条为单位，输入整数'));	
		trbasic('预约每条房源的费用',"rulenew[price]",empty($rule['price']) ? 0 : $rule['price'],'text',array('validate' => makesubmitstr("rulenew[price]",0,0,0,'','float'),'guide' => '以元为单位'));	
		trbasic('可预约天数',"rulenew[yyday]",empty($rule['yyday']) ? 0 : $rule['yyday'],'text',array('validate' => makesubmitstr("rulenew[yyday]",1,0,0,'','float'),'guide' => '以天为单位'));
