...
$chid_fy = $_REQUEST['chid_fy'] ? $_REQUEST['chid_fy']:2;
$oL = new cls_archives(array(
'chid' => $chid,//模型id，必填
'url' => "?entry=$entry$extend_str".'&chid_fy='.$chid_fy,//表单url，必填，不需要加入chid及pid
'pre' => '',//默认的主表前缀
'from' => " {$tblprefix}".atbl($chid)." a LEFT JOIN {$tblprefix}".atbl($chid_fy)." b ON a.pid38 = b.aid where b.chid=".$chid_fy,//sql中的FROM部分
