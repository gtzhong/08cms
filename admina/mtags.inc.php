...
#                    addslashes(_tag2code($mtag, true))
	...
	if(!submitcheck('bmtagsedit')){
		...
		$str = '';;
		if(($tclass || $ttype == 'rtag') && empty($templatebase)) $str .= "&nbsp; >><a href=\"?entry=mtags&action=mtagadd&sclass=$sclass&ttype=$ttype&mtagnew[tclass]=$tclass\" onclick=\"return floatwin('open_mtagsedit',this)\">添加</a>";
	...
	if(!submitcheck('bmtagsdetail') && !submitcheck('bmtagcode')){
		...
		if(empty($_nobutton)){
			...
			($ttype != 'rtag' ? "&nbsp; &nbsp; &nbsp; &nbsp; <input class=\"button\" type=\"submit\" name=\"bmtagcode\" value=\"生成代码\" onclick=\"this.form.target='mtagcodeiframe'; _08_resubmit = 1;\">" : '').
