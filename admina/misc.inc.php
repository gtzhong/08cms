...
if($action == 'cronedit'){
	...
	if(!submitcheck('submitcron')  && !submitcheck('bsubmitcron')){
		...
		$cy_arr = array("<input class=\"checkbox\" type=\"checkbox\" name=\"chkall\" onclick=\"checkall(this.form, 'selectid','chkall')\">",array('任务名称','txtL'),'启用','时间','上次执行时间','下次执行时间','删?','管理');
		trcategory($cy_arr);
	...
	if(!submitcheck('cronadd')){
		...
		if(empty($filenameadd) || !$objcron->isFile($filenameadd)) cls_message::show('任务执行文件不存在！请重新填写。',M_REFERER);
